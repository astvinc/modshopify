...
switch ($options[xPDOTransport::PACKAGE_ACTION]) {
    ...
    case xPDOTransport::ACTION_UPGRADE:
        ...
        /* multipassSecret */
        $setting = $object->xpdo->getObject('modSystemSetting',array('key' => 'modshopify.multipass_secret'));
        if ($setting != null) {
            $setting->set('value',$options['multipassSecret']);
            $setting->save();
        } else {
            $object->xpdo->log(xPDO::LOG_LEVEL_ERROR,'[modShopify] multipassSecret setting could not be found, so the setting could not be changed.');
        }
